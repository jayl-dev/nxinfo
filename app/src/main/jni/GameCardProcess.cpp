...
#include <android/log.h> // Include for LOGI
...
void nstool::GameCardProcess::processRootPfs()
{
	...
	// [DEBUG] Log parameters before creating gc_fs_raw
	LOGI("XCI Hdr: PartitionFsAddress=0x%llx, ValidDataEndPage=0x%llx, gc_fs_raw_length=0x%llx",
		(unsigned long long)mHdr.getPartitionFsAddress(),
		(unsigned long long)mHdr.getValidDataEndPage(),
		(unsigned long long)(pie::hac::GameCardUtil::blockToAddr(mHdr.getValidDataEndPage()+1) - mHdr.getPartitionFsAddress())
	);
    ...
    try {
        LOGI("  Instantiating PfsProcess for XCI root HFS0/PFS0 (length=0x%llx)", (unsigned long long)gc_fs_raw->length());
        mRootPfsProc = std::make_shared<nstool::PfsProcess>();
        mRootPfsProc->setInputFile(gc_fs_raw);
        mRootPfsProc->setCliOutputMode(mCliOutputMode);
        mRootPfsProc->setVerifyMode(mVerify);

        LOGI("  Calling PfsProcess.process() for XCI root HFS0/PFS0");
        mRootPfsProc->process();
        LOGI("  PfsProcess.process() completed for XCI root HFS0/PFS0.");

        mFileSystem = mRootPfsProc->getFileSystem();
        LOGI("  FileSystem obtained from PfsProcess.");

        mFsProcess.setInputFileSystem(mFileSystem);
        mFsProcess.setFsFormatName("PartitionFs");
        const auto& pfsHdr = mRootPfsProc->getPfsHeader();
        mFsProcess.setFsProperties({
            fmt::format("Type:      {:s}", pie::hac::PartitionFsUtil::getFsTypeAsString(pfsHdr.getFsType())),
            fmt::format("DirNum:    {:d}", pfsHdr.getFileList().empty() ? 0 : pfsHdr.getFileList().size()), // Simplified count as full dir list not needed
            fmt::format("FileNum:   {:d}", pfsHdr.getFileList().size())
        });
        LOGI("  PfsProcess Header: Type=%s, FileNum=%zu", pie::hac::PartitionFsUtil::getFsTypeAsString(pfsHdr.getFsType()).c_str(), pfsHdr.getFileList().size());
	} catch (const std::bad_alloc& e) {
        LOGE("  Out of memory during PfsProcess for XCI root HFS0/PFS0: %s", e.what());
        throw; // Re-throw to propagate the error
    } catch (const std::exception& e) {
        LOGE("  Exception during PfsProcess for XCI root HFS0/PFS0: %s", e.what());
        throw; // Re-throw to propagate the error
    }
