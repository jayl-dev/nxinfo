...
{
	LOGI("Importing base key file: %s", keyfile_path.to_string().c_str());
	std::shared_ptr<tc::io::FileStream> keyfile_stream;
	try {
		keyfile_stream = std::make_shared<tc::io::FileStream>(tc::io::FileStream(keyfile_path, tc::io::FileMode::Open, tc::io::FileAccess::Read));
	} catch (tc::io::FileNotFoundException& e) {
		LOGE("Key file not found: %s", keyfile_path.to_string().c_str());
		return;
	} catch (tc::Exception& e) {
		LOGE("Failed to open key file: %s (%s)", keyfile_path.to_string().c_str(), e.error());
		return;
	}
	...
	LOGI("Parsed %zu keys from file.", keyfile_dict.size());
	
	// Debug: Print all parsed keys
	for (const auto& pair : keyfile_dict) {
		LOGI("Key: %s", pair.first.c_str());
	}
	...
	if (keyfile_dict.count(key) > 0) { \
		...
		if (val.empty() == false) { \
			...
			if (dec_val.size() != sizeof(tmp_aes128_xtskey)) \
				LOGE("Key: \"%s\" has incorrect length (expected %zu, got %zu)", key.c_str(), sizeof(tmp_aes128_xtskey), dec_val.size()); \
			else { \
				...
				/* LOGI("Loaded XTS key: %s", key.c_str()); */ \
			} \
		} \
	} \
	}
...
#define _SAVE_RSAKEY(key_name, dst, bitsize) \
	...
	if (keyfile_dict.count(key_mod) > 0) { \
		val_mod = keyfile_dict[key_mod]; \
		if (keyfile_dict.count(key_prv) > 0) val_prv = keyfile_dict[key_prv]; \
	...
	std::vector<std::string> kXciHeaderBase = { "xci_header", "gamecard_header", "gamecard" };
	std::vector<std::string> kXciInitialDataBase = { "xci_initial_data", "gamecard_initial_data" };
	std::vector<std::string> kXciCertBase = { "xci_cert", "gamecard_cert" };
	...
	for (size_t name_idx = 0; name_idx < kNameVariantNum; name_idx++)
	{
		...
		if (name_idx < kXciHeaderBase.size())
		{
            ...
            // [FIX] Fallback for simple "header_key" if no specific XCI keys were found
            if (name_idx == kXciHeaderBase.size() - 1 && xci_header_key.empty()) {
                std::string fallbackKey = "header_key";
                if (keyfile_dict.count(fallbackKey) > 0) {
                    std::string val = keyfile_dict[fallbackKey];
                    if (!val.empty()) {
                        tc::ByteData dec_val = tc::cli::FormatUtil::hexStringToBytes(val);
                        // XCI Header Key is AES-CBC (16 bytes)
                        if (dec_val.size() == 16) {
                            aes128_key_t tmp_key;
                            memcpy(tmp_key.data(), dec_val.data(), 16);
                            xci_header_key[isDev ? pie::hac::gc::KekIndex_Dev : pie::hac::gc::KekIndex_Prod] = tmp_key;
                            LOGI("Loaded fallback AES key for XCI (exact match): %s", fallbackKey.c_str());
                        } else if (dec_val.size() == 32) {
                            // If we have a 32-byte header_key (likely XTS for NCA), try using the first 16 bytes for XCI
                            aes128_key_t tmp_key;
                            memcpy(tmp_key.data(), dec_val.data(), 16);
                            xci_header_key[isDev ? pie::hac::gc::KekIndex_Dev : pie::hac::gc::KekIndex_Prod] = tmp_key;
                            LOGW("Loaded fallback AES key for XCI (truncated 32->16): %s", fallbackKey.c_str());
                        } else {
                            LOGW("Found 'header_key' but size %zu != 16 or 32. Skipping as XCI fallback.", dec_val.size());
                        }
                    }
                }
            }
